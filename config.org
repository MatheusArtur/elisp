...
** Nice "pretty simbols" for lambda, nil, comparison operands, etc..
#+BEGIN_SRC emacs-lisp
  (when window-system
...
** Dependencies
   ...
   - Some fonts, I personally use the =terminus-font=, =noto-cjk= and =ttf-fonts= packages so X can load pretty much anything.
